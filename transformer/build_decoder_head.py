...
class VisionTransformerUpHead(BaseDecodeHead):
    ...
    def __init__(self, img_size=512, embed_dim=1024,
                 norm_layer=partial(nn.LayerNorm, eps=1e-6), norm_cfg=dict(type='BN2d', requires_grad=True),
        ...
        if self.num_conv == 2:
            ...
            _, self.syncbn_fc_0 = 0, torch.nn.BatchNorm2d(256)
            _, self.syncbn_fc_1 = 0, torch.nn.BatchNorm2d(256)
            _, self.syncbn_fc_2 = 0, torch.nn.BatchNorm2d(256)
            _, self.syncbn_fc_3 = 0, torch.nn.BatchNorm2d(256)
